...
void InitUARTs(){
    ...
    SIM_SCGC5 |= SIM_SCGC5_PORTA_MASK; // Make sure clock for PORTB is enabled
	...
	UART0_C2 = UARTLP_C2_RE_MASK | UARTLP_C2_TE_MASK | UART_C2_RIE_MASK | UARTLP_C2_RWU_MASK; // Enable Transmitter, Receiver, Receive interrupt
	set_irq_priority(INT_UART0-16,0);
	enable_irq(INT_UART0-16);
	
	UART0_C5 |= UART0_C5_RDMAE_MASK | UART0_C5_TDMAE_MASK; // enable DMA request for RX and TX transfer
